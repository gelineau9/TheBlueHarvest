...
// Validation schema for media posts
...
type MediaPostInput = z.infer<typeof mediaPostSchema>;
...
export function MediaForm({ onSuccess, onCancel }: MediaFormProps) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<MediaPostInput>({
    resolver: zodResolver(mediaPostSchema),
    defaultValues: {
      title: '',
      description: '',
      tags: [],
  ...
  const onSubmit = async (data: MediaPostInput) => {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          placeholder="Add context to your screenshots or media..."
          rows={4}
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="Enter tags separated by commas (e.g., screenshot, landscape, moria)"
          className="border-amber-300 focus:border-amber-600 focus:ring-amber-600 bg-white"
          disabled={isSubmitting}
        />
      ...
      <div className="flex items-center justify-between p-4 bg-amber-50 border border-amber-200 rounded-lg">
        <div>
          ...
          <p className="text-sm text-amber-600">
            {isPublished ? 'This media will be visible to everyone.' : 'Only you can see drafts.'}
          </p>
        </div>
        <button
          type="button"
