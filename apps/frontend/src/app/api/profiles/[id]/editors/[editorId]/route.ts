/**
 * Profile Editor Delete API Proxy Route
 *
 * Proxies editor removal requests to the backend.
 * Route:
 *   DELETE /api/profiles/[id]/editors/[editorId] - Remove an editor
 */
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string; editorId: string }> }) {
  try {
    ...
    const response = await fetch(`${API_CONFIG.BACKEND_URL}/api/profiles/${id}/editors/${editorId}`, {
      method: 'DELETE',
      headers: {
    ...
    console.error('Error removing profile editor:', error);
