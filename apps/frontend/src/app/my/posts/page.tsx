...
export default function MyPostsPage() {
  ...
  const fetchPosts = async (
    ...
    try {
      ...
      const response = await fetch(`/api/users/me/posts?${params}`);
  ...
  const getPostIcon = (typeName: string) => {
    const Icon = POST_TYPE_ICONS[typeName.toLowerCase()] || FileText;
    return Icon;
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold text-amber-900 mb-2">My Posts</h1>
              <p className="text-amber-700">Manage posts you own or have been given permission to edit</p>
            </div>
        ...
        {!isLoading && posts.length === 0 && (
          <div className="text-center py-12">
            ...
            <h2 className="text-xl font-semibold text-amber-800 mb-2">No posts found</h2>
            <p className="text-amber-600 mb-6">
              {filter === 'all'
                ? "You haven't created any posts yet."
                : filter === 'owned'
                  ? "You don't own any posts yet."
                  : "You don't have editor access to any posts."}
            </p>
            <Link href="/posts/create">
              <Button className="bg-amber-600 hover:bg-amber-700 text-white">
                <Plus className="h-4 w-4 mr-2" />
        ...
        {!isLoading && posts.length > 0 && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts.map((post) => {
              const Icon = getPostIcon(post.type_name);
              return (
                  ...
                  <Card className="bg-white border-amber-200 hover:border-amber-400 hover:shadow-md transition-all duration-200 h-full">
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-center gap-2 text-sm text-amber-600">
                        <span className="capitalize">{post.type_name}</span>
                        <span>•</span>
